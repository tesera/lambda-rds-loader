                ...
                .then(function(response) {
                    parse(response.Body.toString(), {delimiter: inputInfo.delimiter}, function(error, output) {
                        ...
                        for(var i = 0; i < output[0].length; i++) {
                            columns.push(output[0][i] + " TEXT NULL");
                        }
    	...
    	return new Q.promise(function(resolve, reject) {
            ...
            pg.connect(inputInfo.connectionString, function(error, client) {
                if(error) {
                    ...
                    var orgTable = tableName;
                    if(i !== 0 && inputInfo.useSingleTable) {
                        tableName = inputInfo.schema + ".temp";
                    }

                    console.log('Loading file:', i+1, 'of', inputInfo.records.length, ' - ', key, 'table:', tableName);
                            ...
                            if(i < inputInfo.records.length) {
                                if(i > 0 && inputInfo.useSingleTable) {
                                        columns = _.map(columns, function(column) {
                                            return column + ' = f1.' + column;
                                        });

                                        var updateSql = "UPDATE " + orgTable + " SET " + columns.join(',') + " FROM " + orgTable + " as t1 LEFT JOIN " + tableName + " as f1 ON t1.featid = f1.featid";
                                        var insertSql = "INSERT INTO " + orgTable + " SELECT f1.* FROM " + tableName + " f1 LEFT JOIN " + orgTable + " t1 ON f1.featid = t1.featid WHERE t1.featid IS NULL";

                                        db.query("WITH upsert AS (" + updateSql + ") " + insertSql)
                                            .then(function() {  
                                                fetchRecord(i);
                                            });    
                                } else {
                    ...
                    if(error) {
                        ...
                        if(i === 0 && inputInfo.useSingleTable) {
                            ...
                            for(var j = 0; j < columns.length; j++) {
                                ...
                                alterColumns.push(db.query(addColumnSql));
                                newColumns.push(columns[j] + " TEXT NULL");
                            }

                            alterColumns.push(db.query("DROP TABLE IF EXISTS " + inputInfo.schema + ".temp; CREATE TABLE IF NOT EXISTS " + inputInfo.schema + ".temp" + "(" + newColumns.join(',') + ") WITH (OIDS=FALSE);"));

                            Q.allSettled(alterColumns)
                                .then(function() { 
                                    return importRecords(i, key, columns, tableName);
                                });
