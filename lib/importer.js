var region = process.env['AWS_REGION'];
if (!region || region === null || region === "") {
    region = "us-east-1";
    console.log("AWS Lambda RDS Database Loader using default region " + region);
}

var aws = require('aws-sdk');
aws.config.update({
    region : region
});
var s3 = new aws.S3({
    apiVersion : '2006-03-01',
    region : region
});
var dynamoDB = new aws.DynamoDB({
    apiVersion : '2012-08-10',
    region : region
});

var lawgs = require('lawgs'); 
lawgs.config({
    aws: {
        region: region
    }
});

require('./constants');
var common = require('./common');
var kmsCrypto = require('./kmsCrypto');
kmsCrypto.setRegion(region);
var S3S = require('s3-streams');
var pgCopy = require('pg-copy-streams').from;
var pg = require('pg');
var parse = require('csv-parse');
var Q = require('q');
var pgp = require('pg-promise')({ promiseLib: Q });
var db;
var _ = require('underscore');
var getS3 = Q.nbind(s3.getObject, s3);
var s3Keys = require('./s3-keys');
var logger  = lawgs.getOrCreate('/aws/ecs/lambda-rds-loader');

function Importer(records, debug) {
	var _this = this;
	_this.records = records;
    _this.defaultTableName = null;

    if(!debug) {
        console.log('Log to CloudWatch');
        var date = new Date();
        var logStream = date.getFullYear() + '/' + (date.getMonth() + 1) + '/' + date.getUTCDate() + '/' + (Math.random().toString(36).substr(2) + Math.random().toString(36).substr(2));
        console.log = function(d) {
            logger.log(logStream, d);
        }
    }
};

Importer.prototype.getS3Records = function(bucket, prefix) {
	var _this = this;
    var deferred = Q.defer();
	var records = [];
	var prefix = prefix.replace(/^\/|\/$/g, '') + '/';

	var s3ListKeys = new s3Keys(s3, 1000);
		s3ListKeys.listKeys({
  			bucket: bucket,
  			prefix: prefix
		}, function (error, keys) {
  			if (error) {
    			deferred.reject(error);
  			}

  			_.each(keys, function (key) {
  				if(_.last(key).replace(/[\W_]+/g,'').length !== 0) {
  					records.push({
			            s3: {
			                bucket: {
			                    name: bucket,
			                },
			                object: {
			                    key: key
			                }
			            }
        			});
    			}
  			});
  			deferred.resolve(records);
	});

	return deferred.promise;
},

Importer.prototype.getTableName = function(key, inputInfo) {
    var table = key.split('/');
    if(inputInfo.folderDepthLevelForTableName < 0) {
        table.reverse();
    folderDepthLevelForTableName = (inputInfo.folderDepthLevelForTableName * -1) - 1;
    }
    return inputInfo.schema + '.' + inputInfo.tablePrefix + table[folderDepthLevelForTableName].replace(/[\W]+/g,'_');  
};

Importer.prototype.getTableNames = function(inputInfo) {
    return new Q.promise(function(resolve, reject) {
        if(!inputInfo.useSingleTable) {
            for(var i = 0; i < inputInfo.records.length; i++) {
                var key = inputInfo.records[i].s3.object.key;
                var tableName = Importer.prototype.getTableName(key, inputInfo);
                if(inputInfo.tableNames.indexOf(tableName) === -1) {
                    inputInfo.tableNames.push({
                        'tableName': tableName,
                        'key': key
                    });
                    console.log("Resolving table name to: " + tableName);
                }
            }                
        }

        resolve(inputInfo);
    });
};

Importer.prototype.createTablesIfNotExists = function(inputInfo) {
        var createTables = [];
        var table = inputInfo.tableNames[0];
        var deferred = Q.defer();

        var createTableIfNotExists = function(table) {    
            var createDeferred = Q.defer();
            getS3({Bucket: inputInfo.bucket, Key: table.key})
                .then(function(response) {
                    parse(response.Body.toString(), {delimiter: inputInfo.delimiter}, function(error, output) {
                        var port = parseInt(inputInfo.connectionString.port);
                        var unloggedTable = inputInfo.unloggedTables && port !== 5439 ? 'UNLOGGED' : '';

                        var sql = "CREATE " + unloggedTable + " TABLE IF NOT EXISTS " + table.tableName + "(";
                        var columns = [];
                        inputInfo.current.maxSelect = [];

                        for(var i = 0; i < output[0].length; i++) {
                            columns.push(output[0][i] + " text");
                            inputInfo.current.maxSelect.push('max(' + output[0][i] + ') as ' + output[0][i]);
                            inputInfo.columns[output[0][i]] = true;
                        }
                        sql += columns.join(',') + ')';

                        // redshift requires OID
                        if(port !== 5439) {
                            sql += ' WITH (OIDS=FALSE);';
                        }
                        console.log(sql);
                          
                        db.query(sql)
                            .then(function() {
                                createDeferred.resolve();
                            })
                            .catch(function(error) {
                                console.log(error);
                                createDeferred.reject(error);
                            });
                    });
                });
                return createDeferred.promise;
        };

        for(var i = 0; i < inputInfo.tableNames.length; i++) {
            createTables.push(createTableIfNotExists(inputInfo.tableNames[i]));
        }

        Q.allSettled(createTables).then(function(response) {
            deferred.resolve(inputInfo);
        })

        return deferred.promise;

    };

Importer.prototype.updateConfig = function(inputInfo) {
    console.log("Found RDS Configuration for " + inputInfo.bucket);

    var config = inputInfo.config.Item;
    var decryptMap = Q.nbind(kmsCrypto.decryptMap, kmsCrypto);
    var deferred = Q.defer();
    var encryptedItems = {
        'rdsPassword': kmsCrypto.stringToBuffer(config.loadRDS.L[0].M.connectPassword.S),
        'awsAccessKeyId': kmsCrypto.stringToBuffer(config.loadRDS.L[0].M.awsAccessKeyId.S),
        'awsSecretAccessKey': kmsCrypto.stringToBuffer(config.loadRDS.L[0].M.awsSecretAccessKey.S)
    };

    decryptMap(encryptedItems)
        .then(function(response) {
            inputInfo.tablePrefix = config.loadRDS.L[0].M.tablePrefix.S;
            inputInfo.schema = config.loadRDS.L[0].M.targetSchema.S;
            inputInfo.folderDepthLevelForTableName = config.folderDepthLevelForTableName.N;
            inputInfo.truncateTarget = config.loadRDS.L[0].M.truncateTarget.BOOL;
            inputInfo.unloggedTables = config.loadRDS.L[0].M.unloggedTables.BOOL;
            inputInfo.useSingleTable = config.loadRDS.L[0].M.useSingleTable.BOOL;
            inputInfo.connectionString = {
                host: config.loadRDS.L[0].M.rdsHost.S,
                port: config.loadRDS.L[0].M.rdsPort.N,
                database: config.loadRDS.L[0].M.rdsDB.S,
                user: config.loadRDS.L[0].M.connectUser.S,
                password: response.rdsPassword.toString()
            };
            inputInfo.awsAccessKeyId = response.awsAccessKeyId.toString();
            inputInfo.awsSecretAccessKey = response.awsSecretAccessKey.toString();

            db = pgp(inputInfo.connectionString);

            if('filenameFilterRegex' in config) {
                var patt = new RegExp(config.filenameFilterRegex.S);
                inputInfo.records = _.filter(inputInfo.records).filter(function(record) {
                    return patt.test(record.s3.object.key);
                });
            }

            if(inputInfo.records.length === 0) {
                deferred.reject('After regex filter there were no files left. Check regex and try again.');
            } else {
                deferred.resolve(inputInfo);
            }
        });
    return deferred.promise;
};

Importer.prototype.dropTablesIfExists = function(inputInfo) {
        var dropTables = [];
        var deferred = Q.defer();

        var dropTableIfExists = function(table) {
            var sql = "DROP TABLE IF EXISTS " + table;
            console.log(sql);
            return db.query(sql);
        };

        if(inputInfo.truncateTarget) {
            for(var i = 0; i < inputInfo.tableNames.length; i++) {
                dropTables.push(dropTableIfExists(inputInfo.tableNames[i].tableName));
            }
        }

        Q.allSettled(dropTables).then(function() {
            deferred.resolve(inputInfo);
        });

        return deferred.promise;
    };

Importer.prototype.runImport = function(inputInfo) {        
    console.log('Importing data.');
    var deferred = Q.defer();
    var _this = this;
    var importRecords = importRDSRecords;
    if(parseInt(inputInfo.connectionString.port) === 5439) {
        importRecords = importRedshiftRecords;
    }

    function aggregateRecords(key, columns) {
        return new Q.promise(function(resolve, reject) {
            // before doing anything on the insert check if the table can be aggregated
            var keyPath = key.split('/');
            var keyParts = _.last(keyPath).split('_'); 

            if(inputInfo.useSingleTable && inputInfo.current.column != null && (inputInfo.current.column != keyParts[0] || inputInfo.current.row != keyParts[1])) {
                inputInfo.current.column = keyParts[0];
                inputInfo.current.row = keyParts[1];

                var aggregateSql = "INSERT INTO " + inputInfo.tableNames[0].tableName + "_final (" + inputInfo.current.maxSelect.join(',').replace(/max\((.*?)\) as /g,'') + ")" + " SELECT " + inputInfo.current.maxSelect.join(',') + " FROM " + inputInfo.tableNames[0].tableName + " GROUP BY featid; TRUNCATE TABLE " + inputInfo.tableNames[0].tableName + ";";
                console.log('Aggregating data.');
                console.log(aggregateSql);

                db.query(aggregateSql)
                    .then(function() {
                        resolve();
                    })
                    .catch(function(error) {
                        reject(error);
                    });

            } else if(inputInfo.current.column == null) {
                inputInfo.current.column = keyParts[0];
                inputInfo.current.row = keyParts[1];
                resolve();
            } else {
                resolve();
            }
        });
    }

    function importRedshiftRecords(i, key, columns, tableName) {
        return new Q.promise(function(resolve, reject) {
            aggregateRecords(key).then(function() {
                console.log('Loading file: ' + (i+1) + ' of ' + inputInfo.records.length + ' - ' + key);

                //HEADER
                var copySql = "COPY " + tableName + " (" + columns.join(',') + ") FROM 's3://" + (inputInfo.bucket + '/' + key) + "' CREDENTIALS 'aws_access_key_id=" + inputInfo.awsAccessKeyId + ";aws_secret_access_key=" + inputInfo.awsSecretAccessKey + "' IGNOREHEADER 1 CSV DELIMITER '" + inputInfo.delimiter + "'";

                db.query(copySql)
                    .then(function() {
                        i++;
                                            
                        if(i < inputInfo.records.length) {
                            fetchRecord(i);
                        } else {
                            inputInfo.current.column = inputInfo.current.row = -1;

                            return aggregateRecords(key, columns).then(function() {
                                if(inputInfo.defaultTableName && inputInfo.useSingleTable) {
                                    db.query('ALTER TABLE ' + inputInfo.tableNames[0].tableName + '_final RENAME TO ' + inputInfo.defaultTableName)
                                        .then(function() {
                                            resolve(inputInfo);
                                            deferred.resolve(inputInfo);
                                        })
                                        .catch(function(error) {
                                            console.log(error);
                                            resolve(inputInfo);
                                            deferred.resolve(inputInfo);
                                        })
                                } else {
                                    resolve(inputInfo);
                                    deferred.resolve(inputInfo);
                                }         
                            });
                        }
                    })
                    .catch(function(error) {
                        console.log(error);
                        deferred.reject(error);
                    })
            });
        });         
    }

    function importRDSRecords(i, key, columns, tableName) {
    	return new Q.promise(function(resolve, reject) {

            aggregateRecords(key).then(function() {
                var stream = S3S.ReadStream(s3, {Bucket: inputInfo.bucket, Key: key});
                pg.connect(inputInfo.connectionString, function(error, client) {
                    if(error) {
                        reject(error);
                    } else {
                        console.log('Loading file: ' + (i+1) + ' of ' + inputInfo.records.length + ' - ' + key);

                        var copySql = "COPY " + tableName + " (" + columns.join(',') + ") FROM STDIN WITH CSV HEADER DELIMITER '" + inputInfo.delimiter + "'";

                        var query = client.query(pgCopy(copySql));
    	                stream.pipe(query)
    	                    .on('end', function () {
                                client.end();
                                i++;
    	                                    
                                if(i < inputInfo.records.length) {
                                    fetchRecord(i);
                                } else {
                                    inputInfo.current.column = inputInfo.current.row = -1;

                                    return aggregateRecords(key, columns).then(function() {
                                        if(inputInfo.defaultTableName && inputInfo.useSingleTable) {
                                            db.query('ALTER TABLE ' + inputInfo.tableNames[0].tableName + '_final RENAME TO ' + inputInfo.defaultTableName)
                                                .then(function() {
                                                    resolve(inputInfo);
                                                    deferred.resolve(inputInfo);
                                                })
                                                .catch(function(error) {
                                                    console.log(error);
                                                    resolve(inputInfo);
                                                    deferred.resolve(inputInfo);
                                                })
                                        } else {
                                            resolve(inputInfo);
                                            deferred.resolve(inputInfo);
                                        }         
                                    });
                                }
                            })
    	                    .on('error', function(error) {
    	                        console.log('STREAM ERROR. RETRYING.');
                                console.log(error);
                                console.log(copySql);
                                importRecords(i, key, columns, tableName);
    	                    });
                    }
                });
            }); 
        }); 
    }

    function fetchRecord(i) {
    	var key = inputInfo.records[i].s3.object.key;
        var tableName = Importer.prototype.getTableName(key, inputInfo);
        var columns = [];

        getS3({Bucket: inputInfo.bucket, Key: key})
            .then(function(response) {
                parse(response.Body.toString(), { delimiter: inputInfo.delimiter, columns: true }, function(error, output) {
                    if(error) {
                        deferred.reject(error);
                    } else {
                        columns = _.keys(output[0]);

                        var _this = Importer.prototype;
                        var tableName = _this.getTableName(key, inputInfo);
                        

                        if(i === 0 && inputInfo.useSingleTable) {
                            console.log("Resolving table name to: " + tableName);
                            
                            inputInfo.tableNames.push(
                                {
                                    'tableName': tableName,
                                    'key': key
                                },
                                {
                                    'tableName': tableName + '_final',
                                    'key': key
                                }
                            );

                            _this.dropTablesIfExists(inputInfo)
                                .then(_this.createTablesIfNotExists)
                                .then(function(response) {
                                    return importRecords(i, key, columns, tableName);
                                })
                                .catch(function(error) {
                                    deferred.reject(error);
                                });

                        } else if(inputInfo.useSingleTable) {
                            tableName = inputInfo.tableNames[0].tableName;
                            var newColumns = [];
                            for(var j = 0; j < columns.length; j++) {
                                if(typeof inputInfo.columns[columns[j]] === 'undefined') {
                                    inputInfo.current.maxSelect.push('max(' + columns[j] + ') as ' + columns[j]);
                                    inputInfo.columns[columns[j]] = true;
                                    newColumns.push(db.query("ALTER TABLE " + tableName + " ADD COLUMN " + columns[j] + " TEXT NULL; ALTER TABLE " + tableName + "_final" + " ADD COLUMN " + columns[j] + " TEXT NULL;"));
                                }
                            }

                            Q.allSettled(newColumns)
                                .then(function() { 
                                    importRecords(i, key, columns, tableName);
                                });
                                    

                        } else {
                            importRecords(i, key, columns, tableName);
                        }
                    }    
                });
            });
    }

    fetchRecord(0);
    return deferred.promise;
};

Importer.prototype.getConfig = function() {
    var _this = this;
    return new Q.promise(function(resolve, reject) {
    	var getDynamoItem = Q.nbind(dynamoDB.getItem, dynamoDB);
        var inputInfo = {
            bucket: _this.records[0].s3.bucket.name,
            key: _this.records[0].s3.object.key,
            schema: 'public',
            folderDepthLevelForTableName: 0,
            tableNames: [],
            connectionString: null,
            delimiter: ',',
            truncateTarget: false,
            tablePrefix: '',
            defaultTableName: _this.defaultTableName,
            useSingleTable: false,
            sourceColumn: 'rds_loader_source_file',
            records: _this.records,
            current: {
                row: null,
                column: null,
                maxSelect: [],
                baseSelect: []
            }, 
            columns: {}
        };

        // load the configuration for this prefix
        var dynamoLookup = {
            Key : {
                s3Prefix : {
                    S : inputInfo.bucket
                }
            },
            TableName : configTable,
            ConsistentRead : true
        };

        return getDynamoItem(dynamoLookup)
            .then(function(response) {
                inputInfo.config = response;
                resolve(inputInfo);
            })
            .catch(function(error) {
                console.log(error);
                reject(error);
            });
    });
};

Importer.prototype.run = function() {
	var _this = this;
    var deferred = Q.defer();

    _this.getConfig()
        .then(_this.updateConfig)
        .then(_this.getTableNames)
        .then(_this.dropTablesIfExists)
        .then(_this.createTablesIfNotExists)
        .then(_this.runImport)
        .catch(function(error) {
            if(error.stack !== undefined) {
                console.log(error.stack);
            } else {
                console.log(error);
            }
            deferred.reject(1);
        })
        .finally(function() {
            pgp.end();
            deferred.resolve(0);
        });

    return deferred.promise;
};

module.exports = Importer;
