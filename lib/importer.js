...
Importer.prototype.getColumns = function(key, inputInfo) {
    var deferred = Q.defer();
            ...
            .then(function(response) {
                parse(response.Body.toString(), { delimiter: inputInfo.delimiter, columns: true }, function(error, output) {
                    if(error) {
                        ...
                        deferred.resolve(Object.keys(output[0]));
                    }
                });
            });
            ...
            getS3({Bucket: inputInfo.bucket, Key: table.key})
                .then(function(response) {
                    parse(response.Body.toString(), {delimiter: inputInfo.delimiter}, function(error, output) {
                        ...
                        var columns = [inputInfo.serialColumn + " serial"];
                        for(var i = 0; i < output[0].length; i++) {
                            columns.push(output[0][i] + " text");
                        }
                        columns.push(inputInfo.sourceColumn + " text");
                        sql += columns.join(',') + ')';
                        console.log(sql);
...
Importer.prototype.runImport = function(inputInfo) {        
            ...
            pg.connect(inputInfo.connectionString, function(error, client) {
                if(error) {
	                ...
	                var query = client.query(pgCopy(
	                   "COPY " + tableName + " (" + columns.join(',') + ") FROM STDIN WITH CSV HEADER DELIMITER '" + inputInfo.delimiter + "'"
	                ));
	                stream.pipe(query)
	                    .on('end', function () {
	                        console.log('Loaded file:', i+1, 'of', inputInfo.records.length, ':', key);
	                        var setSourceSql = "UPDATE " + tableName + " SET " + inputInfo.sourceColumn + " = '" + key + "' WHERE " + inputInfo.sourceColumn + " IS NULL";
	                        db.query(setSourceSql)
	                            .then(function() {
	                                client.end();
	                                i++;
    ...
    function fetchRecord(i) {
		...
		stream.on('data', function(data) {
            ...
            parse(data.toString(), { delimiter: inputInfo.delimiter, columns: true }, function(error, output) {
                if(error) {
                    deferred.reject(error);
                } else {
                    ...
                    if(i === 0 && inputInfo.useSingleTable) {
                        ...
                        for(var j = 0; j < columns.length; j++) {
                            var addColumnSql = "ALTER TABLE " + tableName + " ADD COLUMN " + columns[j] + " TEXT DEFAULT NULL";
